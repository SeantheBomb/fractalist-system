---
name: "Reading the Room"
epithet: ""
kinship: "dyad"
order: 19
personifies: "The Nearest Light and The Mirror, meeting in two people."
tending: "The feeling in the room is real data."
fraying: "You've mistaken your reaction for their state."
drawable: true
parents: ["the-nearest-light", "the-mirror"]
relatedThirds: ["the-nearest-light", "the-mirror", "the-hall-of-mirrors"]
---

**A fusion of [The Nearest Light](/thirds/the-nearest-light/) and [The Mirror](/thirds/the-mirror/)**, meeting in two people. A figure's tier is what it's made of, not a rank it was given.

**Tending.** Emotion read as genuine, usable data.

**Fraying.** Your own reaction mistaken for the room's state.

**Its Turning.** **The Weather Report** — each names what they think the other feels, then gets corrected. What it builds: projection separates from perception.

Catalogued in [The Dyad-Kin](/node/dyad-kin/), alongside every other figure at this scale.
